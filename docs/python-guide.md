...
| 文件/参数                             | 能否修改      | 说明                                    |
| --------------------------------- | --------- | ------------------------------------- |
| `config/grade_thresholds.yaml`    | **主要改这里** | `grade.boundaries`（Pan 1–5）、`color_classification`（色相 + 深浅三档）、定位参数 |
| `color_classification.hue.*`（yaml 内）  | 可改        | CIELAB 色相阈值（蓝/绿/棕）      |
| `color_classification.depth.boundaries`  | 可改        | L* 深浅三档边界（浅/中/深），与 grade 独立      |
...
- `Grade 1–5` 使用 `grade.boundaries` 中的 L* 明度，目标是近似 Pan 2017 风格深浅分档。
- `iris_color` 分两步：先用 `color_classification.hue`（a*/b*）定蓝/绿/棕，再用 `color_classification.depth.boundaries`（L*）定浅/中/深，共 9 种标签（如浅蓝色、蓝色、深蓝色等）。
- **Grade 边界与颜色深浅边界相互独立**，可在 yaml 中分别标定。
...
2. 记录每张的 L* 中位数
3. 分别调整 `grade.boundaries` 与 `color_classification.depth.boundaries`
4. 对比自动分级与人工分级的一致性
